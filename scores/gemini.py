...
for i in range(num_test):
    ...
    with open(f"gemini_index_{i}_responses_kfolds.pkl", "wb") as f:
        pickle.dump(responses, f)
    prompt_strings = [prompt[0] for prompt in prompts]
    with open(f"gemini_index_{i}_prompts_kfolds.pkl", "wb") as f:
        pickle.dump(prompt_strings, f)
    targets = [prompt[1] for prompt in prompts]
    with open(f"gemini_index_{i}_targets_kfolds.pkl", "wb") as f:
    ...
    for w in range(1, len(prompt)):
        while True:
            try:
                ...
                print(e)
                time.sleep(1)
    with open(f"gemini_index_{i}_responses_inference.pkl", "wb") as f:
        pickle.dump(responses, f)
    with open(f"gemini_index_{i}_prompts_inference.pkl", "wb") as f:
        pickle.dump(prompt, f)
    with open(f"gemini_index_{i}_targets_inference.pkl", "wb") as f:
